...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([apiFetch("/api/users"), apiFetch("/api/students")]).then(async ([uRes, sRes]) => {
      const uJson = await uRes?.json();
      const sJson = await sRes?.json();
      if (uRes?.ok) setUsers(uJson.data); else setError("Failed to load data");
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {students.map((s) => (
                <tr key={s._id}>
                  <td style={{ fontWeight: 500 }}>{s.userId?.name || "—"}</td>
