"use client";

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import { apiFetch, getToken } from "../../lib/api";

export default function StudentDashboard() {
  const router = useRouter();
  const [profile, setProfile]   = useState(null);
  const [grades, setGrades]     = useState([]);
  const [schedule, setSchedule] = useState([]);
  const [tab, setTab]           = useState("profile");
  const [loading, setLoading]   = useState(true);

  useEffect(() => {
    if (!getToken()) { router.replace("/login"); return; }
    Promise.all([apiFetch("/api/student/profile"), apiFetch("/api/student/grades"), apiFetch("/api/student/schedule")])
      .then(async ([pRes, gRes, sRes]) => {
        if (pRes?.ok) setProfile(await pRes.json());
        if (gRes?.ok) setGrades((await gRes.json()).grades || []);
        if (sRes?.ok) setSchedule((await sRes.json()).schedule || []);
        setLoading(false);
      });
  }, []);

  if (loading) return <Spinner />;
  if (!profile) return <div className="alert alert-error">Profile not found. Ask your admin to link your account to a student record.</div>;

  const { student, user } = profile;
  const avg = grades.length ? Math.round(grades.reduce((s, g) => s + g.score, 0) / grades.length) : null;
  const scoreColor = (s) => s >= 80 ? "var(--green)" : s >= 60 ? "var(--yellow)" : "var(--red)";

  return (
    <div style={{ maxWidth: 760 }}>
      {/* Profile header */}
      <div className="card" style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
        <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(16,185,129,0.15)", color: "#10b981", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 22, flexShrink: 0 }}>
          {user.name.charAt(0).toUpperCase()}
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ fontWeight: 700, fontSize: 16, color: "var(--text)" }}>{user.name}</p>
          <p style={{ fontSize: 12, color: "var(--text-2)", marginTop: 2 }}>{user.email}</p>
        </div>
        <div style={{ textAlign: "right" }}>
          <span className="badge badge-green">{student.gradeLevel}</span>
          {avg !== null && <p style={{ fontSize: 12, color: "var(--text-2)", marginTop: 6 }}>Avg: <strong style={{ color: scoreColor(avg) }}>{avg}</strong></p>}
        </div>
      </div>

      {/* Tabs */}
      <div className="tabs" style={{ marginBottom: 16 }}>
        {["profile","grades","schedule"].map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`tab ${tab === t ? "active" : ""}`}>{t}</button>
        ))}
      </div>

      {tab === "profile" && (
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          {[["Full name", student.name], ["Grade level", student.gradeLevel], ["Email", user.email], ["Parent", student.parent?.name || "—"], ["Attendance records", student.attendance?.length ?? 0]].map(([k, v]) => (
            <div key={k} className="profile-row" style={{ padding: "12px 20px" }}>
              <span className="profile-key">{k}</span>
              <span className="profile-value">{v}</span>
            </div>
          ))}
        </div>
      )}

      {tab === "grades" && (
        <div className="table-wrap">
          {grades.length === 0
            ? <p style={{ padding: 20, color: "var(--text-2)", fontSize: 13 }}>No grades recorded yet.</p>
            : <table>
                <thead><tr><th>Subject</th><th>Term</th><th>Score</th><th>Note</th></tr></thead>
                <tbody>
                  {grades.map((g, i) => (
                    <tr key={i}>
                      <td style={{ fontWeight: 500 }}>{g.subject}</td>
                      <td className="td-muted">{g.term}</td>
                      <td><span style={{ fontWeight: 700, color: scoreColor(g.score) }}>{g.score}</span></td>
                      <td className="td-muted">{g.note || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          }
        </div>
      )}

      {tab === "schedule" && (
        <div className="table-wrap">
          {schedule.length === 0
            ? <p style={{ padding: 20, color: "var(--text-2)", fontSize: 13 }}>No schedule set yet.</p>
            : <table>
                <thead><tr><th>Day</th><th>Subject</th><th>Teacher</th><th>Start</th><th>End</th></tr></thead>
                <tbody>
                  {schedule.map((s, i) => (
                    <tr key={i}>
                      <td><span className="badge badge-purple">{s.day}</span></td>
                      <td style={{ fontWeight: 500 }}>{s.subject}</td>
                      <td className="td-muted">{s.teacher}</td>
                      <td className="td-muted">{s.startTime}</td>
                      <td className="td-muted">{s.endTime}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          }
        </div>
      )}
    </div>
  );
}

function Spinner() {
  return <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--text-2)" }}><span className="spinner" /> Loading…</div>;
}
